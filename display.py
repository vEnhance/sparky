...
PROMPT_TEXT = TERM_COLOR["BOLD_CYAN"] + "sparky" \
...
def print_bets(bets, **kwargs):
	if kwargs['weight'] is not None:
		...
		for w in config.WEIGHTS:
			valid_bets = [format_bet(bet) for bet in bets \
					if is_valid(bet, **kwargs) and bet['weight']==w]
			if len(valid_bets) == 0: continue
...
def format_bet(bet):
	...
	return "{ci}{i:>3}.  {outcome} {name:<50} {reset}{cdate}{date}{reset}".format(
			ci = TERM_COLOR["CYAN"], cdate = TERM_COLOR["MAGENTA"],
			reset = TERM_COLOR["RESET"], i = bet['index'],
			date = format_time(bet['birthdate']), name = bet['name'],
			outcome = the_outcome)

def print_stats(generator):
	for payload in generator:
		weight, accuracy, num_correct, num_incorrect = payload
		print("{:>2}%   {ca}{:>7.2%}  {cc}{:>3} {cw}{:>3}{reset}".format(
				*payload,
