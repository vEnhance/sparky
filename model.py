...
class Manager:
	def __init__(self, path = None):
		self.path = path
		try:
			with open(self.path, "r") as f: self.bets = yaml.load(f)
		except IOError:
			self.bets = None
			self.save()
			print("Created {} because it was not found.".format(self.path))
		if self.bets is None: self.bets = []
		self.commit()
	def save(self):
		"""Saves the current bets into the YAML file."""
		with open(self.path, "w") as f: f.write(yaml.dump(self.bets))
	def commit(self):
	...
	def add_bet_arg(self, data):
		...
		assert data['weight'] in config.WEIGHTS,\
				"Invalid Weight {0} not in {1}"\
				.format(data['weight'], config.WEIGHTS)
	...
	def get_outcome_tier(self, weight):
		...
		return [bet for bet in self.bets
				if bet['weight'] == weight
				and bet['outcome'] is not None]
	def provide_stats(self, limit):
		for w in config.WEIGHTS:
			candidate_bets = [bet for bet in self.bets if bet['weight'] == w
					and bet['outcome'] is not None][-limit:]
			num_correct = len([bet for bet in candidate_bets
					if bet['outcome'] is True])
			num_incorrect = len([bet for bet in candidate_bets
